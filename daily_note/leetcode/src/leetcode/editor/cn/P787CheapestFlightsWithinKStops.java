...
public class P787CheapestFlightsWithinKStops{
    public static void main(String[] args) {
        ...
        System.out.println(solution.findCheapestPrice(3, arr, 0, 2, 1));
    }
...
class Solution {
    public int findCheapestPrice(int n, int[][] flights, int src, int dst, int k) {
        ...
        //arr[x][y]表示经过x步从起点走到y的费用
        int[][] arr = new int[k+2][n];
        for (int i = 0; i < k + 2; i++) {
        ...
        for (int i = 1; i < k + 2; i++) {
            for (int[] flight : flights) {
                ...
                int c = flight[2];
                //循环放入下一步的费用
                arr[i][y] = Math.min(arr[i][y], arr[i - 1][x] + c);
            }
        }
        int ans = INF;
        for (int i = 1; i < k + 2; i++) {
            ans = Math.min(ans, arr[i][dst]);
        }
