...
public class P797AllPathsFromSourceToTarget{
    ...
    class Solution {
        public List<List<Integer>> allPathsSourceTarget(int[][] graph) {
            ...
            temp.add(0);
            handle(graph, graph[0], temp, result, graph.length - 1);
            return result;
        }

        private void handle(int[][] graph, int[] graphNode, List<Integer> temp, List<List<Integer>> result, int n) {
            for (int i : graphNode) {
                temp.add(i);
                if (i == n) {
                ...
                handle(graph, graph[i], temp, result, n);
